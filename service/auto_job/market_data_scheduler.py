...
_job_status = {
    ...
    "time_data_total": _persisted.get("time_data_total", 0),
    "time_data_success": _persisted.get("time_data_success", 0),
    "time_data_failed": _persisted.get("time_data_failed", 0),
    "order_book_total": _persisted.get("order_book_total", 0),
    "order_book_success": _persisted.get("order_book_success", 0),
    "order_book_failed": _persisted.get("order_book_failed", 0),
    "dragon_tiger_count": _persisted.get("dragon_tiger_count", 0),
    "running": False,
    "error": None,
}
...
async def _execute_job(manual=False):
    from service.auto_job.scheduler_orchestrator import scheduler_lock, manual_semaphore, market_data_done_event
    if manual:
        _job_status["running"] = True
        _job_status["error"] = "等待手动调度槽位..."
        logger.info("[盘后数据调度] 手动触发，等待调度槽位")
        async with manual_semaphore:
            _job_status["error"] = None
            logger.info("[盘后数据调度] 已获取手动调度槽位")
            try:
