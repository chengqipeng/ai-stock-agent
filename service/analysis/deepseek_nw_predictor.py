...
1. 本周涨幅>大盘涨幅+2%（相对强势）
2. 连续两周上涨
3. 前周跌>5%但本周跌幅<1%或微涨（企稳反弹）
4. 放量上涨（vol_ratio>1.2且本周涨）
5. 60日低位(<0.25)且本周跌幅明显收窄

### 看跌信号（每个+1分）
1. 本周跌幅>大盘跌幅+3%（相对弱势，注意阈值更高）
2. 连续两周下跌且跌幅加速
3. 高位(>0.8)且放量下跌
4. 大盘涨>1%但个股跌>2%（独立弱势）
5. 连跌4天+最后一天仍跌
...
- 看跌分=1且看涨分=0 → UNCERTAIN（看跌需要更强证据）
- 其他 → UNCERTAIN
...
## 重要提醒
- 单周下跌不是看跌理由（下周反弹概率>50%）
- 逆势大涨的股票倾向继续涨
- 不确定就UNCERTAIN，但不要超过50%的股票都UNCERTAIN

```json
{"direction":"UP","confidence":0.62,"justification":"逆势放量上涨+低位企稳"}
```"""
...
def _build_user_prompt(code: str, stock_name: str, features: dict) -> str:
    ...
    if mkt_prev is not None:
        ...
        if mkt_prev < -1 and mkt_this < -1:
            parts.append("- ⚠️ 大盘环境: 连续两周下跌，明确下跌趋势")
        elif mkt_prev > 1 and mkt_this > 1:
            parts.append("- 📈 大盘环境: 连续两周上涨，明确上涨趋势")
        elif (mkt_prev < -1 and mkt_this > 0.5) or (mkt_prev > 1 and mkt_this < -0.5):
            parts.append("- ⚠️ 大盘环境: 可能处于拐点，方向不明确")
        else:
    ...
    if prev is not None:
        ...
        if prev < -1 and this_chg < -1:
            parts.append(f"- ⚠️ 两周趋势: 连续两周下跌（前周{prev:+.1f}%，本周{this_chg:+.1f}%）")
        elif prev > 1 and this_chg > 1:
            parts.append(f"- 📈 两周趋势: 连续两周上涨（前周{prev:+.1f}%，本周{this_chg:+.1f}%）")
        elif prev < -8:
            parts.append(f"- ⚠️ 前周暴跌{prev:+.1f}%，恐慌情绪可能延续")
