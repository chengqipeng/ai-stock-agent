...
def _get_analysis_header(stock_code: str, stock_name: str, mode: str = "full") -> str:
    ...
    if mode == "analyse":
        return (
            f"# 1. 使用欧奈尔CAN SLIM规则分析一下<{stock_code} {stock_name}>，是否符合买入条件：基于模型的最终判断，稳健买入价格区间：基于技术形态（如杯柄形态、突破点）等进行深度行业调研给出的建议\n"
             "# 2.分析涉及当日交易信息、当日资金流向、股票基本信息、业绩报表、高管减持、机构持仓变化、、10/50/200日均线等数据必须严格使用已提供的【东方财富数据集】\n"
    ...
    return (
        ...
        "# 1.分析涉及当日交易信息、当日资金流向、股票基本信息、业绩报表、高管减持、机构持仓变化、10/50/200日均线等数据必须严格使用已提供的【东方财富数据集】\n"
