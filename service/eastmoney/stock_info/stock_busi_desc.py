...
async def get_stock_core_theme(secucode="002371.SZ"):
    """获取股票核心主题"""
    ...
    params = {
        ...
        "quoteColumns": "",
        "filter": f'(SECUCODE="{secucode}")(IS_POINT="1")(IS_HISTORY="0")',
        "pageNumber": "1",
        "pageSize": "",
        "sortTypes": "1,1",
    ...
    if data.get("result") and data["result"].get("data"):
        ...
        raise Exception(f"未获取到股票 {secucode} 的核心主题数据")


async def get_stock_board_type(secucode="002371.SZ"):
    """获取股票板块类型"""
    url = "https://datacenter.eastmoney.com/securities/api/data/v1/get"
    params = {
    ...
    if data.get("result") and data["result"].get("data"):
        ...
        raise Exception(f"未获取到股票 {secucode} 的板块类型数据")


if __name__ == "__main__":
    result = asyncio.run(get_stock_board_type("002371.SZ"))
    print(result)
