...
async def get_real_main_fund_flow(stock_info: StockInfo, date: str = None):
    ...
    if data.get("data") and data["data"].get("diff"):
        ...
        for stock in data["data"]["diff"]:
            ...
            data_date = date or (datetime.fromtimestamp(ts).strftime('%Y-%m-%d') if ts else datetime.now().strftime('%Y-%m-%d'))
            stock_info_dict = {
                "股票名称": stock_info.stock_name,
                "日期": data_date,
