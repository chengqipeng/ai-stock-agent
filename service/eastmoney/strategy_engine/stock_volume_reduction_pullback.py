...
def identify_volume_reduction_pullback(df: pd.DataFrame, vol_ma_window=50, vol_ratio=2.0) -> pd.DataFrame:
    """
    缩量回调不用慌：识别健康洗盘形态
...
_CN_COLUMNS = {
    ...
    'volume': '成交量（万）', 'ma20': 'MA20', 'defense_line': '防守线',
...
async def get_volume_reduction_pullback(stock_info: StockInfo, limit=400, vol_ma_window=50, vol_ratio=2.0) -> pd.DataFrame:
    ...
    return identify_volume_reduction_pullback(df, vol_ma_window, vol_ratio)
...
async def get_volume_reduction_pullback_cn(stock_info: StockInfo, limit=400, vol_ma_window=50, vol_ratio=2.0) -> dict:
    ...
    cols = ['open', 'close', 'high', 'low', 'volume', 'ma20', 'defense_line', 'prev_high_vol']
