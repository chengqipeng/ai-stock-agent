...
def calculate_macd_signals(df: pd.DataFrame) -> pd.DataFrame:
    ...
    prev_bear_min_price, prev_bear_min_dif = float('inf'),  float('inf')
    prev_bull_max_price, prev_bull_max_dif = float('-inf'), float('-inf')
    ...
    for i in range(1, len(data)):
        ...
        if data['Golden_Cross'].iloc[i]:
            if state == -1:
                if (curr_min_price < prev_bear_min_price) and (curr_min_dif > prev_bear_min_dif):
                    if prev_bear_min_price != float('inf') and curr_min_idx != -1:
                        data.iloc[curr_min_idx, bot_col] = True
                prev_bear_min_price = curr_min_price
                prev_bear_min_dif   = curr_min_dif
            state = 1
            curr_max_price, curr_max_dif, curr_max_idx = price_high, dif, i

        elif data['Death_Cross'].iloc[i]:
            if state == 1:
                if (curr_max_price > prev_bull_max_price) and (curr_max_dif < prev_bull_max_dif):
                    if prev_bull_max_price != float('-inf') and curr_max_idx != -1:
                        data.iloc[curr_max_idx, top_col] = True
                prev_bull_max_price = curr_max_price
                prev_bull_max_dif   = curr_max_dif
            state = -1
            curr_min_price, curr_min_dif, curr_min_idx = price_low, dif, i
