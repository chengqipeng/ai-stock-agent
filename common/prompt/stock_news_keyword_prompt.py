...
### 1. 国内搜索 (针对 get_news)
- `"{company_name} 业绩快报 净利润 订单积压"`
- `"{company_name} 国产替代 招标份额 半导体设备"`
- `"{company_name} 大基金持仓 减持 增持"`

### 2. 北美/全球搜索 (针对 get_global_news)
- `"{company_name} BIS Export Controls Entity List Trump 2.0"`
- `"Semiconductor equipment market share {company_name} vs Applied Materials"`
...
def get_can_slim_news_keyword_prompt(company_name):
    current_date = datetime.now().strftime("%Y-%m-%d")
    return f"""
## 角色设定
你是一名精通**全球半导体产业链**与**金融情报检索**的专家。你的任务是为特定公司（{company_name}）设计精准的搜索关键词，以便获取最能影响股价的技术面、基本面和宏观面信息。
