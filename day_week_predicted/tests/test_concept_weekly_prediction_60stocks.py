...
from day_week_predicted.backtest.concept_weekly_prediction_backtest import (
    run_concept_weekly_backtest,
    run_concept_weekly_backtest_simulated,
)
...
def main():
    ...
    print("=" * 70)
    print("  概念板块增强周预测回测 — 60只股票 × 15个概念板块 × 12+周")
    print("=" * 70)
    ...
    # 运行回测
    # 先尝试DB，不可达则用模拟数据
    # 设置 USE_SIMULATED=True 强制使用模拟数据
    USE_SIMULATED = True  # DB不可达时设为True
    ...
    if db_available:
        print("  数据模式: 数据库（DB）")
        result = run_concept_weekly_backtest(
        ...
        result = run_concept_weekly_backtest_simulated(
            stock_codes=stock_codes,
